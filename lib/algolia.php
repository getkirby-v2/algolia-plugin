...
class Algolia {
  ...
  // Caches
  protected $indexCache = array();
  
  /**
   * Class constructor
   */
  public function __construct() {
    $app = c::get('algolia.app');
   ...
   * @return Collection
   */
  public function search($query, $page = 1, $options = array()) {
    $defaultOptions = c::get('algolia.search.options', array());
  ...
  public function insertPage(Page $page) {
    if(!static::isIndexable($page)) return;
  ...
  public static function isIndexable(Page $page) {
    ...
    if(isset($template['filter'])) {
      ...
      if(is_callable($filter) && !call_user_func($filter, $page)) return false;
    }
  ...
  public static function formatPage(Page $page) {
    $fields = c::get('algolia.fields', array('url', 'intendedTemplate'));
  ...
  protected function getIndexName($temp = false) {
    $index = c::get('algolia.index', 'kirby');
    
    if($temp) {
      return c::get('algolia.index.temp', $index . '_temp');
    } else {
      return $index;
    }
  }
